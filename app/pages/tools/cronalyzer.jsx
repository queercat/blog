...
export default function CronCreator() {
  ...
  function parse(input) {
    if (input != undefined) {
      const words_regex =
        /run\b|every\b|\d+\b|minut(es|e)\b|secon(ds|d)\b|hou(rs|r)\b|da(y|ys)\b|mont(h|hs)\b/g;
          ...
          if (numbers.length == time_units.length && numbers.length > 0) {
            ...
            if (when_specifier[0].value == "every") {
              ...
              time_object.forEach((time) => {
                ...
                carry_map[ordered_times[ordered_times.indexOf(unit) + 1]] =
                  carry;
              });
  ...
  return (
    <div className={styles.cron_container}>
      <NextSeo
        title="cronalyzer"
        description="analyze and create cron schedules with english."
      />
      <h1>{cronOutput}</h1>

      <div className={styles.cron_input_container}>
        <textarea
          placeholder="run every 5 minutes..."
          onInput={(e) => {
            setCronInput(e.target.value.replace("\n", ""));
      ...
      <Aside mood="default">
        i feel compelled to say if you use this somewhere and it breaks its not{" "}
        <b>
          technically <i>my</i> fault.
        </b>
        <br />
      </Aside>
    </div>
